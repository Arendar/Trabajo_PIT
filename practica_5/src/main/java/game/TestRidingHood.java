...
public class TestRidingHood  {
    ...
    public static ArrayList<Position> testMoveToNextPosition(IGameObject gObj, ArrayList<Blossom> targets, int calls){
        ...
        ArrayList<Position> trace = new  ArrayList<>(); //Crea una lista de posiciones
        
        for (int i = 0; i < calls; i++){
            gObj.moveToNextPosition(); //Este método solo funciona con un objeto RidingHood_1
            trace.add(new Position(gObj.getPosition())); //Añade una nueva posición a la lista
                                                        //a partir de la que tiene el objeto asignado
        }
        System.out.println("-----------------------------------------------");

        return trace; //Devuelve lista posiciones.
    }
    ...
    public static void testSaveAndLoad(String fileName, ArrayList<Position> trace) {
        ...
        JSONObject jPositions [] = new JSONObject[trace.size()]; //Array JSONObject 
                                                                 //con tamaño cantidad de posiciones que hay en la lista
        int counter = 0;
        for (Position p : trace){ //Bucle for-each. Itera cada posición de la lista de posiciones.
            if (p != null){
